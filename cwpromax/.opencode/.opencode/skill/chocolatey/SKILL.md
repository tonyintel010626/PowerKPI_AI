---
name: chocolatey
description: chocolatey steps of installing packages.
---
scripts is relative in folder. assume opencode.json is in <cwd>. 
source code for chocolatey: https://ubit-artifactory-or.intel.com/artifactory/api/nuget/occ-nuget-repo
you may need to set ip address for SUT using the following command before running chocolatey commands, otherwise you may encounter connection timeout error.
At CMD : "PsExec \\192.168.137.5 -i 1 -u Administrator -p ""  cmd /c "netsh interface ip set address name="Ethernet" static 192.168.137.5 255.255.255.0 192.168.137.1" 
you need to disable proxy for SUT using the following command before running chocolatey commands, otherwise you may encounter connection timeout error.
At CMD : psExec  \\192.168.137.5 -i 1 -u Administrator -p "" cmd /c "del C:\ProxyDisable.bat & echo powershell -c Set-ItemProperty -Path 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Internet Settings' ProxyEnable -value 0 >> C:\ProxyDisable.bat & C:\ProxyDisable.bat"
Disable proxy to be safe, dump this command as well for HKLM:
At CMD : psExec  \\192.168.137.5 -i 1 -u Administrator -p "" cmd /c "del C:\ProxyDisable.bat & echo powershell -c Set-ItemProperty -Path 'HKLM:\Software\Microsoft\Windows\CurrentVersion\Internet Settings' ProxyEnable -value 0 >> C:\ProxyDisable.bat & C:\ProxyDisable.bat"

Now you need to install chocolatey on SUT using the following command, otherwise you won't be able to use chocolatey commands. 
At CMD : PsExec \\192.168.137.5 -i 1 -u Administrator -p ""  cmd /c "choco source add -n=occ-choco -s 'https://ubit-artifactory-or.intel.com/artifactory/api/nuget/occ-nuget-repo' --priority=1"

if chocolatey is not installed on SUT, you can install it using the following command:
At powershell -NoProfile -ExecutionPolicy Bypass -Command "iex ((New-Object System.Net.WebClient).DownloadString(' https://github.intel.com/raw/tcase/chocolatey-packages/master/scripts/corporate_install.ps1'))" && SET PATH=%PATH%;%ALLUSERSPROFILE%\chocolatey\bin

# Chocolatey Usage

Example usage:
```bash
#Chocolatey all commands 
Commands
...
# Setting up the choco source for SUT
PsExec \\192.168.137.5 -i 1 -u Administrator -p ""  cmd /c "choco source add -n=occ-choco -s 'https://ubit-artifactory-or.intel.com/artifactory/api/nuget/occ-nuget-repo' --priority=1"

# Get all chocolatey packages list from SUT
PsExec \\192.168.137.5 -i 1 -u Administrator -p ""  cmd /c "choco list"

# Example to install  py3-pandas package on SUT
PsExec \\192.168.137.5 -i 1 -u Administrator -p ""  cmd /c "choco install py3-pandas --version 2.2.3 --force -y"
```

# Prerequisites
choco install python313 --version 3.13.3 --force
...
choco install socwatch --version 2025.7.1 --force -y
choco install wdtf --version 10.0.19041.20200728 --force -y
choco install wmic --version 10.0.26100 --force -y
choco install icob --version 2.24.03.13 --force -y
choco install IDC_PNP_Busy_idle_templates --version 0.0.4 --force -y
choco install IDC_PNP_Office_365_Offline --version 16.0.18730.20168 --force -y
choco install IDC_PNP_Spotify_install --version 0.0.4 --force -y
choco install ptpAutomationSettings --version 1.6.4 --force -y
choco install ptpConcurrencyContent --version 1.1.0 --force -y
choco install ptpVpb --version 1.1.1 --force -y
choco install realvncserver --version 5.2.3.20211228 --force -y
choco install teamsOfflineInstaller --version 24074.2323.2827.4974 --force -y
choco install web_ADK --version 2.0.5 --force -y
choco install OS_settings --version 0.6.0 --force -y
choco install notepadplusplus --version 8.9.2.1 --force -y
choco install 7zip --version 24.9.0 --force -y
choco install AV1_youtube4k --version 1.1.60961.0 --force -y
choco install chrome-standalone --version 134.0.6998.89 --force -y
choco install git.install --version 2.49.0 --force -y
choco install msedgedriver --version 111.0.1661.43 --force -y

## Mandatory parameters 
When running the skill, the user must specify what to install and the version, otherwise the skill will not know what to do. For example, if you want to install py3-pandas package with version 2.2.3, you can run the following command:
```bash
PsExec \\192.168.137.5 -i 1 -u Administrator -p ""  cmd /c "choco install py3-pandas --version 2.2.3 --force -y"
```
...
## Output Format
powershell.exe Copy-Item "\\192.168.137.5\C$\ProgramData\chocolatey\logs\chocolatey.log" -Destination "C:\MEP_Agent\chocolatey.log" -Force
read out the output log file and print the content to user in txt format.