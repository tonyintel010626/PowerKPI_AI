...
function build(shape: LogoShape): LogoContext {
  ...
  const MID = shape.mid?.[0]?.length ?? 0
  const FULL = shape.left.map((line, i) => {
    const midPart = shape.mid ? shape.mid[i] + " ".repeat(GAP) : ""
    return line + " ".repeat(GAP) + midPart + shape.right[i]
  })
  const SPAN = Math.hypot(FULL[0]?.length ?? 0, FULL.length * 2) * 0.94
  return { LEFT, MID, FULL, SPAN, MAP: mapGlyphs(FULL), shape }
}
          ...
          <box flexDirection="row" gap={1}>
            ...
            <box flexDirection="row">
              {renderLine(
                ...
                true,
                ctx.LEFT + GAP + (ctx.MID > 0 ? ctx.MID + GAP : 0),
                frame(),
                dusk(),
                idleState(),
              )}
