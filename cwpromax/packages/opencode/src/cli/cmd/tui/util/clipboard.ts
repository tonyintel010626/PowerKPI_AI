...
export async function read(): Promise<Content | undefined> {
  ...
  if (os === "win32" || release().includes("WSL")) {
    ...
    // Read text directly via PowerShell (more reliable than clipboardy on Windows)
    const textResult = await Process.text(
      ["powershell.exe", "-NonInteractive", "-NoProfile", "-Command", "Get-Clipboard"],
      { nothrow: true },
    )
    if (textResult.text?.trim()) {
      return { data: textResult.text.trimEnd(), mime: "text/plain" }
    }
    return undefined
  }
