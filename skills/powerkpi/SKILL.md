...
│  - IDON, CMS, ICOB, Busy Idle, YouTube, Netflix, Teams, S5  │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
...
# Flextune BKM (Best Known Method) — Ver 1.0

## What is Flextune?

Flextune is an automation utility designed to simplify workload execution by leveraging Hopper commands in a controlled and standardized way.

Instead of manually copying, pasting, and modifying Hopper commands for each workload, Flextune centralizes and automates this process, significantly reducing operational overhead.

### Why Flextune is Used

Traditionally, running workloads requires:
- Manual pasting of Hopper commands / Using PACs
- Repeated variable changes for different workloads
- High risk of human error during execution

Flextune eliminates these inefficiencies by handling command execution and variable management automatically.

**Key Benefit:** Engineers no longer need to interact directly with Hopper commands for routine workload execution.

### Key Objectives of Flextune

- Automate Hopper command execution
- Eliminate repetitive manual power capture
- Standardize workload execution across Power Stations
- Allow engineers to focus on debugging and analysis instead of operations
- Improve efficiency in data capturing
...
- Flextune is **pre-installed** on every Power Station
- No manual setup is required before launching
- If not installed, contact the automation engineer in your team

### Launching Flextune

**Step 1: Locate the Shortcut**
- Go to the Desktop
- Locate the shortcut named: `start_tunning....shortcut`

**Step 2: Start Flextune**
- Double-click the shortcut
- Flextune will launch automatically
- Always use the desktop shortcut to ensure the correct environment and configuration are loaded

**Step 3: Open the Tuning Job Interface**
- Once Flextune is launched, click on the tab labeled: **Launching Tuning Job**

---

## Flextune Interface Overview

### 1. Variable Configuration

The Variable Configuration section defines and controls workload parameters that Flextune passes to the Hopper command during execution.

> **Warning:** Direct modification of the configuration file is NOT recommended. Do not manually edit the config file outside Flextune. Incorrect changes may result in Flextune launch failure, workload execution errors, or incorrect data capture.

**Adding or Modifying Workloads:**
1. Use the **Edit** option within the Flextune UI
2. Add new workload entries inside the existing configuration file
3. Save changes and verify they appear correctly in the UI
...
- Loop count
- Target selection
- SoC or DAQ instrumentation

### 2. Variable Info, Instrument and NI-DAQ Self Calibrate

#### FlexLogger Configuration Handling

- The FlexLogger configuration file and command are **not** directly embedded in the workload command list
- FlexLogger configuration changes between projects
- The FlexLogger path and command are defined in the **Instrument tab**
- You can update the path once, without modifying each individual workload command

#### SoCWatch Triggering Method

- Flextune does **not** trigger the standard SoCWatch `.bat` file inside the SUT using psexec as in manual execution
- All required capture IPs are predefined in the Instrument tab
- Flextune triggers SoCWatch using this predefined configuration
- Enables custom capture scenarios, especially helpful for Pre-Silicon team requests

#### Use of Variables in Commands ($ Syntax)

Some commands contain a dollar sign (`$`), for example: `-comm_type $communicator_type`

- The `$` symbol indicates a variable placeholder
- The actual value is defined in the Variable Info tab
- Example: `communicator_type = ps_exec` → resolved to `-comm_type ps_exec`
- This allows the communicator type (`efi`, `ps_exec`, `xml`) to change without editing every command

#### Important Warning – Project Directory

- The project directory variable is pre-configured based on the station product (NVLS / NVLP)
- **Do NOT modify the project directory** — changing this may lead to incorrect file paths, instrument launch failure, or Flextune execution errors

#### NI-DAQ Self Calibrate

- Shortcut to allow NI-DAQ devices to perform self-calibration automatically
- Eliminates the need to manually click DEV A → DEV E and calibrate one by one
- Hopper commands already perform calibration checks during execution
- If there are no visible power measurement errors, this step can be skipped

---

## List of Workloads

### Workload Panel Overview

On the right side of the Flextune UI, you will see a list of available workloads. Each workload represents a predefined Hopper command set. Users only need to **select workloads** — no manual command editing is needed.

### Commonly Used Workloads

| Workload | Description |
|----------|-------------|
| `win_opt` | Windows optimization — typically run before or as part of power tuning runs |
| `Auto Proxy Enable` | Uses auto-detect settings in Windows proxy configuration |
| `Manual Proxy Enable` | Uses fixed proxy (e.g., `http://proxy.png.intel.com`) — preferred for power runs |
| `Proxy Disable` | Disables all proxy settings |

### Workload Capture Types

| Type | Description |
|------|-------------|
| `workload_daq` | Normal power capture using FlexLogger (DAQ only) |
| `workload_soc` | SoCWatch capture only — no power (DAQ) data collected |
| `workload_daq_soc` | Parallel capture of both FlexLogger power data and SoCWatch data |

### Workflow Selection and Execution Priority

- Flextune executes workloads in the **exact order they are selected** in the workload list
- Execution priority is determined by **selection sequence**, not by workload name or type

**Example — if selected in this order:**
1. `cms_daq`
2. `busy_idle_daq`
3. `window_opt`

Flextune executes them in that exact sequence.

### Important Execution Rule

If you need to enable or disable proxy settings before running a workload, you **must select the proxy workload first**, before selecting the actual workload.

Example:
1. Select **Manual Proxy Enable**
2. Then select the target workload (e.g., `idon_daq`)

### Best Practices

- Always verify the selection order before starting execution
- Select environment-setup workloads (proxy enable/disable, win_opt) **before** performance or power workloads

---

## Chapter 2: Workload Running

### Launching a Workload

After completing all setup steps and confirming the selected workloads:

1. **Select** the workloads by ticking them in the required execution sequence
2. **Click the Launch button** to start workload execution

> **Important:** The **Launch Flow** button is NOT encouraged for normal operation. Launch Flow executes the entire predefined flow that was previously saved and may include workloads not required for the current run. Always use **Launch** (not Launch Flow) for controlled execution.

### Finding and Managing Output Files

**Output File Location:** `C:\tuning_workload`

All output files are automatically generated and saved by Flextune after workload execution. Each execution generates an output folder containing all workloads selected and run in that session.

**Output File Naming Logic:**
- File/folder name is automatically created based on execution sequence and time of execution
- Name may not clearly reflect the workload content

**Important Recommendation:** Rename the output file or folder **immediately after workload completion** to avoid confusion during later analysis.

**Example Scenario:**
- Morning run (Netflix + IDON) → one output folder with both datasets
- Afternoon run (YouTube + Catapult browsing) → separate output folder

---

## Chapter 3: Known Bugs and Possible Solutions

### 3.1 File Path Error

**Issue:** Different stations may have different file paths or missing tools/dependencies due to historical manual data collection practices.

**Solution:** Use Chocolatey to standardize tool installation across all stations.

Run the following commands at the **SUT** Command Prompt:

```bash
choco source add -n=occ-choco -s "https://ubit-artifactory-or.intel.com/artifactory/api/nuget/occ-nuget-repo" --priority=1
choco install python313 --version 3.13.3 --force -y
choco install py3-future --version 1.0.0 --force -y
choco install py3-lxml --version 5.3.0 --force -y
choco install py3-numpy --version 2.2.2 --force -y
choco install py3-pandas --version 2.2.3 --force -y
choco install py3-prettytable --version 3.14.0 --force -y
choco install py3-psutil --version 6.1.1 --force -y
choco install py3-pywin32 --version 308.0.0 --force -y
choco install py3-pywinauto --version 0.6.9 --force -y
choco install py3-selenium --version 4.28.1 --force -y
choco install py3-setuptools --version 75.8.0 --force -y
choco install py3-six --version 1.17.0 --force -y
choco install py3-typing-extensions --version 4.12.2 --force -y
choco install py3-urllib3 --version 2.3.0 --force -y
choco install py3-wheel --version 0.45.1 --force -y
choco install py3-wmi --version 1.5.1 --force -y
choco install py3-xmlcli --version 2.2.2 --force -y
choco install python310-defusedxml --version 0.7.1 --force -y
choco install python310-websocket_client --version 1.8.0.20241129 --force -y
```

### 3.2 Communication Related Error

**Issue:** Communicator errors may occur even when Wi-Fi is connected and VNC connection is available. Often related to incorrect gateway or network sharing configuration.

**Solution — At the SUT:**
1. Open Settings → Network and Internet → Ethernet → Edit
2. Set Gateway to: `192.168.137.1`

**Solution — At the Host:**
1. Open Control Panel → Network and Internet → Network and Sharing Center
2. Select the host internet connection (usually Ethernet 2)
3. Click Properties → Sharing tab
4. Check: **"Allow other network users to connect through this computer's internet connection"**

### 3.3 Chocolatey Related Error

**Issue:** Chocolatey command execution fails.

**Solution:** Re-run the source add command and ensure proxy is **disabled** on the SUT before re-running:

```bash
choco source add -n=occ-choco -s "https://ubit-artifactory-or.intel.com/artifactory/api/nuget/occ-nuget-repo" --priority=1
```

### 3.4 Python-Related Error

**Issue:** After full Chocolatey installation, Flextune still reports Python module not found or Python-related execution errors. Usually caused by multiple Python versions installed on the SUT.

**Solution:**
1. At the SUT, open "Add or remove programs"
2. Search for "Python"
3. Ensure only **one Python version** is installed
4. If multiple versions found, uninstall the extra one

**Standard Python Version for Flextune:** Python 3.13

### 3.5 Other Errors

If the above solutions do not resolve the issue, **contact the Automation Engineer** in your team and provide:
