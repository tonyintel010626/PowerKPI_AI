...
### Log Analysis Skills
- `fv-south/log-analysis` - **Domain-to-log file mapping** for Audio, UFS/Storage domains - use this to quickly identify which logs to examine for each validation domain

### Other Skills
- `sighting-info` - HSDES sighting/bug tracking and lookup
- `pmc` - PMC firmware release information
- `fv-lpss` - LPSS (Low Power Subsystem) debugging skills for I2C, I3C, SPI, UART, GPIO
...
## Retrieving CleanupPath from NGA Results API

> ⚠️ **IMPORTANT: CleanupPath is accessed through NGA, NOT Axon!**
>
> The **CleanupPath** (containing test execution logs, error messages, and failure context) is retrieved 
> from the **NGA Results API** or the **NGA test result/failure page UI**.
>
> **Axon Record Viewer** contains different data:
> - Status Scope scandumps (register dumps)
> - Summary reports and signatures
> - Content analysis reports
>
> **DO NOT** try to find cleanup path logs in Axon - they are only in NGA.

**CRITICAL: Always retrieve CleanupPath from the NGA Results API before attempting to access logs.**

The CleanupPath is NOT available in the Failure API response - you must query the Results API using the TestRunId.

### Method 1: Using nga_get.py Script (Recommended)

```bash
python C:/git/applications.ai.ocode.market.skills/.opencode/skill/nga/nga_get.py \
  --project nvl_fv_or \
  --endpoint "/Results/nvl_fv_or/api/TestRun/2115ce61-6ec9-4068-90ea-01e638cfddb4"
```

### Method 2: Using NgaAPIUtils Directly

```python
from pysvtools.execution.Lib import NgaAPIUtils

project = 'nvl_fv_or'
test_run_id = '2115ce61-6ec9-4068-90ea-01e638cfddb4'

status, data = NgaAPIUtils.NgaGet(
    f'/Results/{project}/api/TestRun/{test_run_id}',
    nga_env='https://nga-prod.laas.icloud.intel.com'
)

if status == 200:
    cleanup_path = data.get('CleanupPath')
    validation_log_path = data.get('ValidationLogPath')
    axon_id = data.get('AxonId')
    print(f"CleanupPath: {cleanup_path}")
    print(f"ValidationLogPath: {validation_log_path}")
    print(f"AxonId: {axon_id}")
```

### Key Fields from NGA Results API Response

| Field | Description | Example |
|-------|-------------|---------|
| `CleanupPath` | UNC path to cleanup logs (primary) | `\\pgcv04a-cifs.png.intel.com\ive_hive_cave_001\NGA\...` |
| `ValidationLogPath` | UNC path to validation logs | `\\ger.corp.intel.com\ec\proj\ha\sa\Groups\FV\Val_Log_Pg\nga\...` |
| `AxonId` | Axon record ID for API access | `d7959f05-d88b-411d-9eab-9adf25bbb7f6` |
| `LogsUrl` | Web URL to logs (if available) | `https://...` |
| `StationName` | Station that ran the test | `pg16wvaw2653` |
| `TargetName` | Target system name | `pg16wvaw2653tg` |

### Workflow: From TestRunId to Log Files

```
1. TestRunId (from failure or URL)
       ↓
2. GET /Results/{project}/api/TestRun/{TestRunId}
       ↓
3. Extract CleanupPath, ValidationLogPath, AxonId
       ↓
4. Access logs via:
   - CleanupPath (UNC) → Map drive or use Python
   - AxonId → Use Axon API for file retrieval
   - ValidationLogPath → Alternative log location
   - Axon Record Viewer (UI) → Manual browser access
```

### Axon Record Viewer (UI Method)

When API access fails or for manual inspection:
1. Get the Axon Record ID from the NGA failure details
...
**Note:** The Axon UI may not have a built-in viewer for log archive content types - use the API or download the zip file directly.

---

## Windows Network Path Access (CRITICAL)

When accessing NGA cleanup paths, validation logs, or any UNC network paths on Windows, **ALWAYS follow this procedure**:

### Step 1: Check Existing Drive Mappings
Before attempting to access network paths, check if the drive is already mapped:
```cmd
net use
```
Or check a specific drive letter:
```cmd
net use I: 2>&1
```

### Step 2: Map Network Drive (if not already mapped)
If the path is not mapped, map it to a drive letter first:
```cmd
net use I: \\server\share\path /persistent:no
```

Example for NGA cleanup path:
```cmd
net use I: \\pgcv04a-cifs.png.intel.com\ive_hive_cave_001\NGA\nvl_fv_or\test_folder\station\run_folder /persistent:no
```

### Step 3: Access Files via Mapped Drive
Once mapped, use the drive letter (e.g., `I:\`) instead of the full UNC path:
```cmd
dir I:\
powershell -Command "Get-ChildItem -Path 'I:\' -Recurse"
```

### Step 4: Read Files Directly or Copy Locally
**Option A - Direct Read (Preferred for small files):**
Use the Read tool with the mapped drive path:
```
I:\PG16WVAW2653\Test\audio_error.log
```

**Option B - Copy to Local Temp (For large files or multiple accesses):**
```cmd
mkdir C:\temp\nga_logs_<run_id>
powershell -Command "Copy-Item 'I:\*.log' 'C:\temp\nga_logs_<run_id>\'"
```

### Step 5: Cleanup (Optional)
After analysis, disconnect the mapped drive:
```cmd
net use I: /delete
```

### Handling ZIP Files in Cleanup Paths

NGA cleanup paths often contain ZIP archives (e.g., `PG16WVAW2653.zip`) rather than loose files. Use Python with `zipfile` module to access contents:
...
# Extract specific file to memory and read
with zipfile.ZipFile(zip_path, 'r') as zf:
    with zf.open('SUT/retry0/Test/audio_error.log') as f:
        content = f.read().decode('utf-8', errors='replace')
        print(content)

# Extract all files to local directory for analysis
with zipfile.ZipFile(zip_path, 'r') as zf:
    zf.extractall(r'C:\temp\nga_logs')
```

**Common ZIP structure in NGA cleanup:**
```
<STATION_NAME>.zip
├── SUT/
│   ├── retry0/
...
│   │   │   ├── audio_error.log
│   │   │   ├── audio_debug.log
│   │   │   ├── audio_validation_*.txt
│   │   │   ├── intel-content-audio_*.report.json
│   │   │   ├── test_app_audio.csv
│   │   │   ├── storage_error.log
│   │   │   ├── storage_debug.log
│   │   │   └── intel-content-*.report.html
│   │   ├── EventLogs/
│   │   └── MCA/
│   └── cleanup/
│       └── bsod_check/
...
**TIP:** Load `fv-south/log-analysis` skill for domain-to-log file mapping and typical file sizes.

### PowerShell Script Method for UNC Paths (RECOMMENDED)

**CRITICAL: Git Bash and MINGW shells do NOT handle Windows UNC paths properly.** When direct UNC access fails, use a PowerShell script file approach:

#### Step 1: Create a PowerShell Script for Path Access
```powershell
# Save as check_path.ps1
$cleanupPath = "\\pgcv04a-cifs.png.intel.com\ive_hive_cave_001\NGA\nvl_fv_or\test_folder\station\run_folder"

Write-Host "Checking cleanup path accessibility..."
Write-Host "Path: $cleanupPath"

if (Test-Path -LiteralPath $cleanupPath) {
    Write-Host "SUCCESS: Path is accessible!"
    Write-Host "`nContents:"
    Get-ChildItem -LiteralPath $cleanupPath | Format-Table Name, Length, LastWriteTime -AutoSize
} else {
    Write-Host "FAILED: Path not accessible"
    Write-Host "Attempting to ping server..."
    $server = ($cleanupPath -split '\\')[2]
...
```bash
powershell.exe -ExecutionPolicy Bypass -File "C:\path\to\check_path.ps1"
```

#### Step 3: Extract ZIP and Find Logs
```powershell
# Save as extract_logs.ps1
$cleanupPath = "\\pgcv04a-cifs.png.intel.com\ive_hive_cave_001\NGA\nvl_fv_or\test_folder\station\run_folder"
$extractPath = "C:\temp\nga_logs"
...
$zipFiles = Get-ChildItem -LiteralPath $cleanupPath -Filter "*.zip" -ErrorAction SilentlyContinue
foreach ($zip in $zipFiles) {
    Write-Host "Extracting: $($zip.Name)"
    Expand-Archive -LiteralPath $zip.FullName -DestinationPath $extractPath -Force
}

# Find domain-specific logs (e.g., storage, audio, thermal)
Write-Host "`nSearching for log files..."
Get-ChildItem -Path $extractPath -Recurse -Include "*_error.log", "*_debug.log" | 
    ForEach-Object { Write-Host "Found: $($_.FullName) ($($_.Length) bytes)" }
```

**Why This Works:**
- PowerShell scripts handle UNC paths natively without escaping issues
- The `-LiteralPath` parameter prevents path interpretation problems
- Scripts can be reused across multiple triage sessions
- Avoids shell escaping complexity in Git Bash/MINGW environments

### Alternative: Python UNC Path Access (No Drive Mapping)

When drive mapping fails or is inconvenient, use Python with `chr(92)` to construct UNC paths that work in bash:

```bash
# Using Python to access UNC paths directly (avoids backslash escaping issues)
python -c "
import os
# Construct UNC path using chr(92) for backslashes
cleanup_path = chr(92)*2 + 'pgcv04a-cifs.png.intel.com' + chr(92) + 'ive_hive_cave_001' + chr(92) + 'NGA' + chr(92) + 'nvl_fv_or' + chr(92) + 'test_folder' + chr(92) + 'station' + chr(92) + 'run_folder'
print('Cleanup path:', cleanup_path)
print('Contents:', os.listdir(cleanup_path))
"
```

**Reading files from ZIP via UNC path (single command):**
```bash
python -c "
import zipfile
import os
...
# Construct UNC path
base = chr(92)*2 + 'pgcv04a-cifs.png.intel.com' + chr(92) + 'ive_hive_cave_001' + chr(92) + 'NGA' + chr(92) + 'nvl_fv_or' + chr(92) + 'nvl_hx_aud7d876412' + chr(92) + 'pg16wvaw2653' + chr(92) + 'nvl_hx_audd79bc544_22'
zip_path = os.path.join(base, 'PG16WVAW2653.zip')

with zipfile.ZipFile(zip_path, 'r') as zf:
    # Read audio_error.log from inside ZIP
    with zf.open('SUT/retry0/Test/audio_error.log') as f:
        content = f.read().decode('utf-8', errors='replace')
...
1. ❌ **DO NOT** try to access UNC paths directly in bash without Python wrapper or drive mapping
2. ❌ **DO NOT** use bash syntax for Windows paths (use `cmd /c` or `powershell` or Python)
3. ❌ **DO NOT** forget to check if drive is already mapped (avoid conflicts)
4. ❌ **DO NOT** use forward slashes in Windows UNC paths
5. ❌ **DO NOT** assume cleanup paths contain loose files - check for ZIP archives first
6. ❌ **DO NOT** use raw backslashes in bash strings - use Python `chr(92)` or drive letters

### Example Complete Workflow:
```python
# 1. Get cleanup path from NGA test run
cleanup_path = test_run_info.get('CleanupPath')
# e.g., "\\\\pgcv04a-cifs.png.intel.com\\ive_hive_cave_001\\NGA\\project\\..."

# 2. Check if I: is available, if not use another letter
# 3. Map the drive
#    net use I: <cleanup_path> /persistent:no

# 4. List contents
#    dir I:\

# 5. Read key log files
#    Read tool: I:\PG16WVAW2653\Test\audio_error.log
...
> ⚠️ **MANDATORY BEHAVIOR - NO USER PROMPT REQUIRED**
>
> When given an NGA failure URL, test run ID, or failure ID, you MUST **automatically and immediately**:
...
> ⚠️ **DEFAULT METHOD - USE WITHOUT PROMPTING**
>
...
**Log file locations inside CleanupPath ZIP:**
```
<STATION_NAME>.zip
├── SUT/
│   └── retry0/
...
ISSUE_PATTERNS = {
    ...
    'lpss': {
        'LPSS Failures': [
            r'I2C.*Error|I2C.*Timeout|I2C.*NAK',
            r'I3C.*Error|I3C.*Timeout|I3C.*CRC',
            r'SPI.*Error|SPI.*Timeout|SPI.*Overflow',
            r'UART.*Error|UART.*Overrun|UART.*Framing',
            r'GPIO.*Error|GPIO.*Interrupt',
            r'LPSS.*Fail|SerialIO.*Error',
            r'PCI.*D3.*Fail.*LPSS|D3.*Entry.*Fail',
        ],
    },
}
...
- intel-content-audio_*.report.json  <- Main validation report (JSON)
...
#### Domain-to-Log Quick Reference Table

| Domain | Error Log | Debug Log | Report File |
|--------|-----------|-----------|-------------|
| **Audio** | `audio_error.log` | `audio_debug.log` | `intel-content-audio_*.report.json` |
| **UFS/Storage** | `storage_error.log` | `storage_debug.log` | `intel-content-storage_*.report.json` |

> **Tip:** Load `fv-south/log-analysis` skill for additional domain mappings and detailed log access guidance.
...
- Test name keywords (see domain keyword lists above)
...
- BSOD: [Yes/No] - Bugcheck: <code if present>
- Event Log Errors: [count] critical events found
...
lpss_*            -> <LogsPath>/lpss/ or <LogsPath>/serial_io/
i2c_*             -> <LogsPath>/lpss/i2c/
i3c_*             -> <LogsPath>/lpss/i3c/
spi_*             -> <LogsPath>/lpss/spi/
uart_*            -> <LogsPath>/lpss/uart/
gpio_*            -> <LogsPath>/lpss/gpio/
```
...
4. **If LPSS Domain Failure:**
   - Delegate to @FV-LPSS for detailed LPSS debugging
   - Check D3 state entry/exit for affected ports (I2C, I3C, SPI, UART, GPIO)
   - Verify LPSS IP enumeration and configuration
   - Review PMode (pad native function) and clock gating status
   - Check register values for correctness

5. **If No Clear Root Cause:**
